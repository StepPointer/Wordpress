...
(function($, _, window, document, undefined) {
	...
	ThemifyPageBuilder = {
		...
		init: function() {
			this.cacheElements(); // cache elements
		...
		bindEvents: function() {
			...
			this.cache.$body.on( 'change', '.tf-image-gradient-field .tf-radio-input-container input', function(){
			...
			/* rows */
			this.cache.$body.on('click', '.themify_builder_toggle_row, .themify_builder_row_js_wrapper .themify_builder_row_top .toggle_row', this.toggleRow)
			...
			/* module */
			this.cache.$body.on('click', '.themify_builder .js--themify_builder_module_styling', this.moduleStylingOption)
			...
			if (this.is_touch_device()) {
				this.cache.$body.on('touchstart', '.themify_builder .themify_module_options', this.optionsModule)
			...
			this.cache.$body.on('builderscriptsloaded.themify', function() {
			...
			// module events
			this.cache.$window.resize(function() {
			...
			this.cache.$body.on('click', '.tfl-icon', function(e) {
			...
			this.cache.$body.on('click.aftertbloader', 'a.js-turn-on-builder', this.toggleFrontEdit);
			$('.themify_builder_dup_link a').on('click', this.duplicatePage);
			$('.slide_builder_module_panel').on('click', this.slidePanel);

			if (this.is_touch_device()) {
				this.cache.$body.addClass('touch');
				this.cache.$body.on('touchstart', '.themify_builder_module_front .module', function(e) {
			...
			this.cache.$body.on('click', '#builder_submit_import_form', this.builderImportSubmit)
		...
		bindEventModuleOption: function( event, moduleSettings, $context ){
			...
			// colorpicker
			this.setColorPicker( $context );

			// plupload init
			this.builderPlupload('normal');

			// option binding setup
			this.moduleOptionsBinding( $context );

			// builder drag n drop init
			this.moduleOptionBuilder( $context );

			// tabular options
			if ( $context.find('.themify_builder_tabs').length ) {
				$context.find('.themify_builder_tabs').tabs();
			}

			ThemifyBuilderCommon.Lightbox.rememberRow();
			// "Apply all" // apply all init
			this.applyAll_init( $context );
            ...
            ThemifyBuilderCommon.fontPreview($context,moduleSettings);
		},
		checkUnload: function() {
			/* unload event */
			if (this.cache.$body.hasClass('themify_builder_active')) {
		...
		// "Apply all" // apply all init
		applyAll_init: function($context) {
			var $this = this;
			$context = $context || $('#themify_builder_lightbox_container');
			$context.find('.style_apply_all').each(function() {
				var $val = $(this).val(),
		...
		setColorPicker: function($context) {
			...
			$context.find('.builderColorSelect').each(function() {
				var $minicolors = $(this),
			...
			self.cache.$body.on('blur keyup', '.colordisplay', function(e) {
				var $input = $(this),
					tempColor = '',
			...
			self.cache.$body.on('blur keyup', '.color_opacity', function(e) {
				var $input = $(this),
					tempOpacity = '',
					$minicolors = $input.parent().find('.builderColorSelect'),
					$field = $input.parent().find('.builderColorSelectInput');
				...
				// "Apply all" // verify is "apply all" is enabled to propagate the border color
				//self.applyAll_verifyBorderColor($field, $field.val(), $input.val(), tempOpacity,e.type);
			})
		},
			...
			$('.themify_module_holder').each(function() {
				...
				$(this).sortable({
					...
					create: function(event, ui) {
						self.cache.$body.css('overflow-x', 'inherit');
					}
				});
			});
		...
		setupTooltips: function() {
			var setupBottomTooltips = function() {
				ThemifyPageBuilder.cache.$body.on('mouseover', '[rel^="themify-tooltip-"]', function(e) {
				...
				}).on('mouseout', '[rel^="themify-tooltip-"]', function(e) {
		...
		optionsModule: function(event, isNewModule) {
			...
			var callback = function(response) {
				...
				// Trigger event
				self.cache.$body.trigger('editing_module_option', [moduleSettings, $(response)]);
		...
		moduleOptionsBinding: function ( $context ) {
			var form = $('#tfb_module_settings');
			$context.on( 'change', 'input[data-binding], textarea[data-binding], select[data-binding]',function () {
				var logic = false,
					$self = $( this ),
		...
		duplicateModule: function( event ) {
			event.preventDefault();
			var self = ThemifyPageBuilder,
				$holder = $(this).closest('.themify_builder_module_front'),
				module_slug = $holder.find('.front_mod_settings').data('mod-name'),
				uniqueID = module_slug + '-' + ThemifyBuilderCommon.randNumber(),
				modID = $holder.find('.module').prop('id'),
				$newElems = $holder.clone(true),
				$newStyle = $newElems.find('style'),
				$builder_container = $(this).closest('.themify_builder_content'),
				builder_id = $builder_container.data('postid'),
				startValue = $builder_container[0].innerHTML;

			$newElems.find('.module').prop('id', uniqueID ).removeClass(modID).addClass(uniqueID);
			if( $newStyle.length )
				$newStyle.html( $newStyle.html().replace( new RegExp(modID, 'g'), uniqueID ) );
			$newElems.find('.themify_builder_dropdown_front').hide();
		...
		mediaUploader: function() {
			...
			// Field Uploader
			this.cache.$body.on('click', '.themify-builder-media-uploader', function(event) {
				var $el = $(this),
				...
				if (isRowBgImage || isRowBgVideo) {
					if ($('#hide_attachment_display_settings').length == 0) {
						ThemifyPageBuilder.cache.$body.append('<style id="hide_attachment_display_settings">.media-modal .attachment-display-settings { display:none }</style>');
					}
			...
			// delete button
			this.cache.$body.on('click', '.themify-builder-delete-thumb', function(e) {
		...
		moduleOptionBuilder: function( $context ) {

			// sortable accordion builder
			$context.find(".themify_builder_module_opt_builder_wrap").sortable({
				items: '.themify_builder_row',
				handle: '.themify_builder_row_top',
				axis: 'y',
				placeholder: 'themify_builder_ui_state_highlight',
		...
		updateContent: function(class_hilite, hilite_obj, module_slug, temp_appended_data, previewOnly, form_state) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					// Hook
					self.cache.$body.trigger('builder_load_module_partial', $newElems);
				}
			});
		},
		toggleFrontEdit: function(e) {
			...
			// add body class
			if (!self.cache.$body.hasClass('themify_builder_active')) {
				is_edit = 1;
		...
		toggleFrontEditAjax: function(is_edit, bids) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (is_edit) {
						self.cache.$body.addClass('themify_builder_active themify_builder_front');
						self.newRowAvailable();
						...
						self.cache.$body.removeClass('themify_builder_active themify_builder_front');
					...
					self.cache.$body.trigger('builder_toggle_frontend', is_edit);
		...
		loadContentJs: function() {
			...
			// hook
			this.cache.$body.trigger('builder_load_on_ajax');
		},
		...
		openGallery: function() {
			...
			this.cache.$body.on('click', '.tf-gallery-btn', function(event) {
				...
				if ($('#hide_gallery_settings').length == 0) {
					$self.cache.$body.append('<style id="hide_gallery_settings">.media-modal .gallery-settings { display:none }</style>');
				}
		...
		moduleActions: function() {
			this.cache.$body.on('change', '.module-widget-select-field', function() {
		...
		optionRow: function(e) {
			...
			var self = ThemifyPageBuilder,
				...
				callback = function(response) {
					...
					}).data('entire-data', options).appendTo($form);
					if ('desktop' !== self.activeBreakPoint) {
					...
					if ('object' === typeof options) {
						...
						$.each(options, function(id, val) {
							$form.find('#' + id).val(val);
						});

						$form.find('.tfb_lb_option[type=radio]').each(function() {
					...
					// image field
					$form.find('.themify-builder-uploader-input').each(function() {
					 ...
					 $form.find( '.themify-gradient ' ).each(function(){
					...
					// builder
					$form.find('.themify_builder_row_js_wrapper').each(function() {
					...
					$form.find('.tf-option-checkbox-enable input:checked').trigger('click');

					// plupload init
					self.builderPlupload('normal');

					/* checkbox field type */
					$form.find('.themify-checkbox').each(function() {
						var id = $(this).attr('id');
					...
					self.cache.$body.trigger('editing_row_option', [options]);
		...
		optionColumn: function(e) {
			...
			var self = ThemifyPageBuilder,
				...
				callback = function(response) {
					...
					if ('desktop' !== self.activeBreakPoint) {
						var styleFields = $form.find('.tfb_lb_option').map(function() {
							return $(this).attr('id');
						}).get(), temp_background_type = options.background_type;
						options = _.omit(options, styleFields);
					...
					$form.find('.themify-checkbox').each(function() {
						...
						if (options[id]) {
							...
							// Set the values
							$.each(options[id], function(i, v) {
								$form.find('.tf-checkbox[value="' + v + '"]').prop('checked', true);
							});
						}
					});
					...
					self.cache.$body.trigger('editing_column_option', [options]);
		...
		liveUpdateRow: function(builder_id, sendData, colLocationObj, previewOnly) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					// Hook
					self.cache.$body.trigger('builder_load_row_partial', $newElems);
				}
			});
		},
		...
		responsiveFrame: {
			...
			init: function() {
				...
				ThemifyPageBuilder.cache.$body.addClass('builder-active-breakpoint--' + ThemifyPageBuilder.activeBreakPoint );
				...
				ThemifyPageBuilder.cache.$body.on('builder_toggle_frontend', ThemifyPageBuilder.responsiveFrame.toggleBuilder);
		...
		breakPointSwitcher: function(event) {
			...
			ThemifyPageBuilder.cache.$body.removeClass('builder-active-breakpoint--desktop builder-active-breakpoint--tablet builder-active-breakpoint--tablet_landscape builder-active-breakpoint--mobile')
			...
			if ('desktop' == breakpoint) {
				...
				var $padding_bottom = breakpoint==='mobile'? ThemifyPageBuilder.cache.$body.outerHeight(true) - ThemifyPageBuilder.responsiveFrame.$el.outerHeight(true):-1;
				if($padding_bottom<0){
					$padding_bottom = 180;
				}
				ThemifyPageBuilder.cache.$body.css('padding-bottom',$padding_bottom);
				...
				ThemifyPageBuilder.responsiveFrame.contentWindow.scrollTo(0, ThemifyPageBuilder.cache.$window.scrollTop());
			}
		},
	...
	ThemifyLiveStyling = (function($, jss) {
		...
		function ThemifyLiveStyling() {
			...
			this.data  = [];
			var self = this;
			this.$body.on('builderiframeloaded.themify', function(e) {
		...
		ThemifyLiveStyling.prototype.bindColors = function() {
			...
			this.$context.on('change', 'input.colordisplay', function() {
				...
				self.$body.trigger('themify_builder_color_picker_change', [colorType, hexString]);
			});
			this.$context.on('change', 'input.color_opacity', function() {
				...
				self.$body.trigger('themify_builder_color_picker_change', [colorType, rgbaString]);
			});
			this.$body.on('themify_builder_color_picker_change', function(e, colorType, rgbaString) {
		...
		ThemifyLiveStyling.prototype.bindTextInputs = function() {
			...
			this.$body.delegate(self.style_tab + ' input[type="text"]', 'keyup', function() {
				var $id = $(this).prop('id'),
			...
			this.$body.delegate(self.style_tab + ' [id$="_unit"]', 'change', function() {
		...
		ThemifyLiveStyling.prototype.bindChangesEvent = function() {
			...
			this.$body.delegate(self.style_tab + ' select,' + self.style_tab + ' input[type="radio"],' + self.style_tab + ' .themify-builder-uploader-input', 'change', function() {
				if(!self.isInit){
					return;
				}
