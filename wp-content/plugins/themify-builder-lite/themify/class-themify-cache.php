...
class TFCache {
    ...
    public static function is_cache_activate() {
        $active = (is_user_logged_in() && current_user_can('manage_options')) || is_admin() || themify_builder_get('builder_disable_cache')!== 'enable' ? true : Themify_Builder::is_front_builder_activate();
     ...
     * return boolean
     */
    public static function check_cache($cache_dir, $time = false) {
    ...
    public static function getExpire(){
        static $time = false;
        if(!$time){
            $time = themify_builder_get('setting-page_builder_expiry');
