	...
	class Themify_Builder_Options {
			...
			switch ( $current_tab ) {
				...
				default:
					...
					if ( Themify_Builder_Model::builder_check() ) {
                                                ...
                                                if (themify_builder_get('builder_disable_cache')==='enable' && (!Themify_Builder_Model::is_premium() || TFCache::check_version())) {
                                                        add_settings_field(
                                                                'builder_clear_cache', 
		...
		public function builder_clear_cache() {
			...
			$pre = $this->current_tab . '_';
                        ...
                        $out.='<br/><br/>';
                        $out.= sprintf( '<a href="#" data-action="themify_builder_clear_all_caches" data-confim="'.__( 'This will clear all builder caches. click ok to continue.', 'themify' ).'" data-clearing-text="%s" data-done-text="%s" data-default-text="%s" data-default-icon="ti-eraser" class="button button-secondary js-clear-builder-cache"> <i class="ti-eraser"></i> <span>%s</span></a><br/><span class="description">%s</span>',
